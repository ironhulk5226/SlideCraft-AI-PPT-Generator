...
const SlidesFrame = ({slide, colors, setUpdateSlider}) => {

  const { projectId } = useParams();
  
    // Remove CORS-problematic background images from slide code
    const cleanSlideCode = (code) => {
        if (!code) return code;
        
        // Remove inline background-image styles with external URLs
        let cleanedCode = code.replace(/background-image:\s*url\(['"]https?:\/\/[^)]+['"]\);?/gi, '');
        
        // Remove style attributes that only contain background-image
        cleanedCode = cleanedCode.replace(/style=["']background-image:\s*url\([^)]+\)["']/gi, '');
    ...
    const FINAL_CODE = HTML_DEFAULT
        ...
        .replace("{code}", cleanSlideCode(slide?.code));
    ...
    useEffect(() => {
        ...
        const handleClick = (e) => {
            ...
            // If clicking a different element, clean up the previous one
            ...
            // ✅ Calculate position relative to the slide container (not viewport)
            ...
            // Calculate position relative to the slide container
            // Since we're using absolute positioning, we don't need viewport offset
            setCardPosition({
    ...
    const handleAiSectionChange = async (userAiPrompt) => {
        setLoading(true);
  ...
  `;
    ...
    return (
        <div className='mb-5 relative'>
            ...
            {cardPosition && (
                <FloatingActionTool 
                    key={`${cardPosition.x}-${cardPosition.y}`}
                    position={cardPosition}
                    onClose={() => setCardPosition(null)}
                    loading={loading}
                    handleAiChange={(value) => handleAiSectionChange(value)}
                />
            )}
        </div>
    );
